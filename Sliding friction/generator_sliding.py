...
sliding_pp          = np.linspace(sliding_pp_min, sliding_pp_max, num_studies)
...
for N in N_values:
    ...
    for seed in seeds:
        ...
        for i in range(num_studies):
            ...
            simulation_newpath = os.path.join(study_directory, f"sliding_{sliding_pp[i]:.2f}")
            if not os.path.exists(simulation_newpath):
                os.makedirs(simulation_newpath)
            batch_launch_newpath = os.path.join(study_directory, f"sliding_{sliding_pp[i]:.2f}")
            if not os.path.exists(batch_launch_newpath):
                os.makedirs(batch_launch_newpath)
            mesh_newpath = os.path.join(study_directory, f"sliding_{sliding_pp[i]:.2f}", "mesh")
            if not os.path.exists(mesh_newpath):
                os.makedirs(mesh_newpath)
            inface_newpath = os.path.join(study_directory, f"sliding_{sliding_pp[i]:.2f}", "mesh")
            if not os.path.exists(inface_newpath):
                os.makedirs(inface_newpath)
            toml_path = os.path.join(study_directory, f"sliding_{sliding_pp[i]:.2f}")
            if not os.path.exists(toml_path):
                os.makedirs(toml_path)
            particles_newpath = os.path.join(study_directory, f"sliding_{sliding_pp[i]:.2f}", "mesh")
        ...
        # Launch Slurm Jobs
        #- 
        glob_input = os.path.join(study_directory, "sliding_*")
