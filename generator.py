...
restitution_pp      = np.linspace(restitution_pp_min, restitution_pp_max, num_studies)
    ...
    for seed in seeds:
        ...
        for i in range(num_studies):
            ...
            simulation_data = { "timestep": timestep, 
                               ...
                               "fricPP": fricPP, 
                               "fricPW": fricPW, 
                               "fricRollPP": fricRollPP, 
                               "fricRollPW": fricRollPW, 
                               "restitution_pp": restitution_pp[i], 
            ...
            simulation_newpath = os.path.join(study_directory, f"restitution_{restitution_pp[i]:.2f}")
            if not os.path.exists(simulation_newpath):
                os.makedirs(simulation_newpath)
            batch_launch_newpath = os.path.join(study_directory, f"restitution_{restitution_pp[i]:.2f}")
            if not os.path.exists(batch_launch_newpath):
                os.makedirs(batch_launch_newpath)
            mesh_newpath = os.path.join(study_directory, f"restitution_{restitution_pp[i]:.2f}", "mesh")
            if not os.path.exists(mesh_newpath):
                os.makedirs(mesh_newpath)
            inface_newpath = os.path.join(study_directory, f"restitution_{restitution_pp[i]:.2f}", "mesh")
            if not os.path.exists(inface_newpath):
                os.makedirs(inface_newpath)
            toml_path = os.path.join(study_directory, f"restitution_{restitution_pp[i]:.2f}")
            if not os.path.exists(toml_path):
                os.makedirs(toml_path)
            particles_newpath = os.path.join(study_directory, f"restitution_{restitution_pp[i]:.2f}", "mesh")
